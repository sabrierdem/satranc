...
if ($who === "") {
  ...
  fail("Redo yetkiniz yok.", 403);
}

if (count($state["future"]) < 1) {
  save_state_unlock($fp, $state);
  fail("İleri alınacak hamle yok.");
}
...
// pop next
$next = array_pop($state["future"]);
$state["fen"] = $next["fen"] ?? "start";
$state["pgn"] = $next["pgn"] ?? "";
$state["over"] = !empty($next["over"]);
$state["result"] = $next["result"] ?? "";
$state["winner"] = $next["winner"] ?? "";
$state["reason"] = $next["reason"] ?? "";
if (isset($next["streak"]) && is_array($next["streak"]))
